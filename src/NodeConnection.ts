...
class ElementConnection {
  ...
  when(event: "connected" | "disconnected"): Subscriptable<void> {
    // return this.state.to(connected => connected === (event === "connected"))
    return { subscribe: next => this.state.subscribe(connected => connected === (event === "connected") && next()) }
  }

  /**
   * Scopes subscriptions inside `connectedCallback` to connected events,
   * provides `disconnectedSignal` argument to unsubscribe when disconnected.
   *
   * @example
   *
   * elementConnection.scope(signal => {
   *   window.addEventListener("scroll", () => ..., { signal })
   *   window.when("pointermove").subscribe(() => ..., { signal })
   * })
   */
